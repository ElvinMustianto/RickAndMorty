...
class CharacterSearchPagingSource(
    ...
    sealed class  LocalException(
        ...
        object EmptySearch: LocalException(
            title = "Start typing search!",
        )
        object NoResult: LocalException(
            title = "Whoops!",
            description = "Looks like your search no results"
        )
    }
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Character> {
        ...
        val request = NetworkLayer.apiClient.getCharacterPageSearch(
        ...
        return LoadResult.Page(
            data = request.bodyNullable?.results?.map { characterResponse ->
                CharacterMapper.getCharacter(characterResponse)
            } ?: emptyList(),
            prevKey= previousKey,
